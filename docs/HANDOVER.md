...
| `sw.js` | Service Worker (Offline). **Cache-Version bei CSS/JS/Seiten-Änderungen hochzählen** (aktuell `v30`) |
...
(Click-to-Load, `youtube-nocookie`) · **FAQ** (sichtbar + `FAQPage`-Schema) · **Sprachumschalter** (Header; **Englisch vollständig live** in `en/` — 25 Seiten;
übrige Sprachen „in Vorbereitung". RU/UK/PL/TR folgen. Details/Prozess in BACKLOG.md).
...
- **Nach CSS/JS-/Seiten-Änderung:** SW-`CACHE_NAME` hochzählen (returning users; aktuell `v30`).
